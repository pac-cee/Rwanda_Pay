...
function CardTypeIcon({ type }: { type: Card["type"] }) {
  ...
  return (
    <View style={styles.brandBadge}>
      <Text style={styles.momoText}>AMEX</Text>
    </View>
  );
}

export default function CardView({ card, isSelected = false, compact = false, hideBalance = false }: CardViewProps) {
  const scale = useRef(new Animated.Value(isSelected ? 1 : 0.97)).current;

  useEffect(() => {
    Animated.spring(scale, { toValue: isSelected ? 1 : 0.97, friction: 7, useNativeDriver: true }).start();
  }, [isSelected]);

  return (
    <Animated.View style={[styles.card, { backgroundColor: card.color }, compact && styles.compact, { transform: [{ scale }] }]}>
      ...
      {!compact && <View style={styles.nfcRow}><Feather name="wifi" size={18} color="rgba(255,255,255,0.7)" /></View>}
      ...
      <View style={styles.bottomRow}>
        ...
        {!compact && (
          <View style={styles.balanceBlock}>
            ...
            <Text style={styles.balance}>{hideBalance ? "••••••" : formatBalance(card.balance)}</Text>
          </View>
        )}
      </View>
    </Animated.View>
...
const styles = StyleSheet.create({
  card: { width: "100%", height: 200, borderRadius: 24, padding: 24, overflow: "hidden", justifyContent: "space-between", shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.2, shadowRadius: 16, elevation: 6 },
  compact: { height: 160, padding: 18 },
  circle1: { position: "absolute", width: 200, height: 200, borderRadius: 100, top: -60, right: -40 },
  circle2: { position: "absolute", width: 160, height: 160, borderRadius: 80, bottom: -50, left: -30 },
